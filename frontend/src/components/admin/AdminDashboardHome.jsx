...
const AdminDashboardHome = () => {
  ...
  const operationalNodes = [
    { name: 'Product Catalog', icon: Box, path: '/admin/products', color: '#3b82f6', desc: 'Inventory & Pricing' },
  ...
  return (
    <div className="executive-dashboard animate-fade">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .executive-dashboard { padding: 40px; background: #f8fafc; min-height: 100vh; font-family: 'Inter', sans-serif; }
        
        .hero-section { background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border-radius: 32px; padding: 50px; color: white; display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px; position: relative; overflow: hidden; }
        .hero-section::after { content: ""; position: absolute; top: -50%; right: -10%; width: 500px; height: 500px; background: radial-gradient(circle, rgba(59, 130, 246, 0.1) 0%, transparent 70%); }
        ...
        .hero-content h1 { font-size: 36px; font-weight: 800; margin: 15px 0 10px; letter-spacing: -0.02em; }
        .hero-content p { color: #94a3b8; font-size: 17px; }
        
        .hero-actions { display: flex; gap: 15px; }
        .primary-action { background: #3b82f6; color: white; border: none; padding: 14px 24px; border-radius: 14px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: all 0.2s; }
        .primary-action:hover:not(:disabled) { background: #2563eb; transform: translateY(-2px); }
        .primary-action:disabled { opacity: 0.7; cursor: wait; }
        
        .secondary-action { background: rgba(255,255,255,0.05); color: white; border: 1px solid rgba(255,255,255,0.1); padding: 14px 24px; border-radius: 14px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: all 0.2s; font-size: 14px; }
        .secondary-action:hover { background: rgba(255,255,255,0.1); }

        .kpi-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 40px; }
        .kpi-card { background: white; border-radius: 24px; padding: 25px; border: 1px solid #e2e8f0; transition: all 0.2s; }
        .kpi-card:hover { transform: translateY(-5px); border-color: #cbd5e1; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.05); }
        ...
        .kpi-value { font-size: 28px; font-weight: 800; color: #0f172a; margin: 5px 0 0; }

        .main-operational-grid { display: grid; grid-template-columns: 1fr 380px; gap: 40px; }
        
        .section-header { margin-bottom: 25px; }
        .section-header h3 { font-size: 20px; font-weight: 800; color: #0f172a; margin: 0; }
        .section-header p { margin: 5px 0 0; color: #64748b; font-size: 14px; }

        .nodes-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; }
        .node-card { background: white; padding: 20px; border-radius: 20px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 18px; text-decoration: none; transition: all 0.2s; }
        .node-card:hover { background: #fcfcfc; border-color: #cbd5e1; transform: translateX(5px); }
        .node-icon { width: 56px; height: 56px; border-radius: 16px; display: flex; align-items: center; justify-content: center; }
        .node-info h4 { margin: 0; font-size: 16px; font-weight: 700; color: #1e293b; }
        .node-info p { margin: 4px 0 0; font-size: 12px; color: #94a3b8; font-weight: 500; }
        .node-chevron { margin-left: auto; color: #cbd5e1; }

        .intel-sidebar { display: flex; flex-direction: column; gap: 20px; }
        .intel-card { background: white; border-radius: 28px; border: 1px solid #e2e8f0; padding: 25px; }
        .intel-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #f1f5f9; }
        .intel-header h3 { margin: 0; font-size: 16px; font-weight: 800; color: #0f172a; }
        .intel-header svg { color: #3b82f6; }

        .activity-stream { display: flex; flex-direction: column; gap: 20px; }
        .stream-item { display: flex; gap: 15px; }
        .stream-marker { width: 8px; height: 8px; border-radius: 50%; background: #3b82f6; margin-top: 6px; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
        .stream-content { flex: 1; }
        .stream-text { display: block; font-size: 13px; color: #475569; line-height: 1.5; }
        .stream-time { font-size: 11px; color: #94a3b8; font-weight: 600; margin-top: 5px; display: block; }
        
        .stream-loading, .stream-empty { font-size: 13px; color: #94a3b8; text-align: center; padding: 20px 0; }

        .health-metrics { display: flex; flex-direction: column; gap: 12px; }
        .metric-row { display: flex; justify-content: space-between; align-items: center; font-size: 13px; font-weight: 600; color: #64748b; }
        .status-green { color: #10b981; font-weight: 800; }
        .status-val { color: #0f172a; font-weight: 800; }
